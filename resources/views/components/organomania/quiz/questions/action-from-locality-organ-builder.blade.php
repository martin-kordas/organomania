...
<x-organomania.quiz.questions.question>
    @if ($question->showOrganBuilder())
        ...
        {!! __('Jakou mají varhany <span class="text-decoration-underline">trakturu</span>?') !!}
    @else
        {!! __('Jakou <span class="text-decoration-underline">trakturu</span> mají varhany v lokalitě') !!}
        <strong>{{ $organ->municipality }}, {{ $organ->place }}</strong>?
    @endif
</x-organomania.quiz.questions.question>
