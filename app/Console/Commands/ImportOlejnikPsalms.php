...
use App\Models\OlejnikPsalm;

class ImportOlejnikPsalms extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:import-olejnik-psalms';
     ...
     * @var string
     */
    protected $description = 'Inserts Olejník psalms into table';
    
    // https://www.zaltar.cz/rejstrik.html
    const DATA = <<<EOL
    ...
    public function handle()
    {
        foreach (explode("\n", static::DATA) as $song) {
            $matches = [];
            preg_match('/^OL([0-9a-z]+) – (.+)$/i', trim($song), $matches);
            OlejnikPsalm::create([
